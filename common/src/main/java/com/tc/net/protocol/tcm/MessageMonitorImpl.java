  ...
  private ScheduledFuture<?> startLogging(final Logger logger, int intervalSeconds) {
    ...
    return this.timer.scheduleAtFixedRate(()->{
      ...
      for (TCConnection c : list) {
        ...
        logger.info(formatter.rightPad(60, state.get("localAddress") + " -> " + state.get("remoteAddress")) + formatter.leftPad(20, "batch:" + sent)  + formatter.leftPad(20, "write:" + write) + formatter.leftPad(20, "read:" + recv));
      }
